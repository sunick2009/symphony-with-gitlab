defmodule SymphonyElixir.CoreTest do
  ...
  test "normal worker exit schedules active-state continuation retry" do
    ...
    assert_due_in_range(due_at_ms, scheduled_after_ms, 500, 1_100)
  end
  ...
  test "abnormal worker exit increments retry attempt progressively" do
    ...
    assert_due_in_range(due_at_ms, scheduled_after_ms, 39_500, 40_500)
  end
  ...
  test "first abnormal worker exit waits before retrying" do
    ...
    assert_due_in_range(due_at_ms, scheduled_after_ms, 9_000, 10_500)
  end
  ...
  defp assert_due_in_range(due_at_ms, scheduled_after_ms, min_delay_ms, max_remaining_ms) do
    observed_ms = System.monotonic_time(:millisecond)
    remaining_ms = due_at_ms - observed_ms

    assert due_at_ms >= scheduled_after_ms + min_delay_ms
  ...
  test "app server startup command supports codex args override from workflow config" do
    ...
    try do
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        codex_command: "#{codex_binary} --config 'model=\"gpt-5.1-codex-mini\"' --config model_reasoning_effort=medium app-server"
      )
      ...
      assert String.contains?(
               argv_line,
               "--config model=\"gpt-5.1-codex-mini\" --config model_reasoning_effort=medium app-server"
             )
